...
pub(crate) fn vertex_block_opts(
    ...
    opts.set_block_size(storage_opts.vertex_block_size);
    opts.set_cache_index_and_filter_blocks(storage_opts.cache_index_and_filter_blocks);
    if storage_opts.cache_index_and_filter_blocks {
...
pub(crate) fn vertex_cf_opts(storage_opts: &RocksOptions, block_opts: &BlockBasedOptions) -> Options {
    let mut opts = Options::default();
    opts.set_block_based_table_factory(block_opts);
    opts.set_write_buffer_size(storage_opts.write_buffer_size);
    opts.set_max_write_buffer_number(storage_opts.max_write_buffer_number);
    opts
}

/// Block-based options shared by edges_out and edges_in CFs (without block cache).
pub(crate) fn edge_block_opts(storage_opts: &RocksOptions, block_cache: Option<&rocksdb::Cache>) -> BlockBasedOptions {
    let mut opts = BlockBasedOptions::default();
    // full filter (not block-based) so prefix seeks hit the bloom filter
    opts.set_bloom_filter(BLOOM_FILTER_BITS_PER_KEY, false);
    opts.set_format_version(BLOCK_FORMAT_VERSION);
    opts.set_block_size(storage_opts.edge_block_size);
    opts.set_cache_index_and_filter_blocks(storage_opts.cache_index_and_filter_blocks);
    if storage_opts.cache_index_and_filter_blocks {
        opts.set_pin_l0_filter_and_index_blocks_in_cache(true);
    }
...
pub(crate) fn edge_cf_opts(storage_opts: &RocksOptions, block_opts: &BlockBasedOptions) -> Options {
    let mut opts = Options::default();
