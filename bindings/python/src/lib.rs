    ...
    match value {
        ...
        Value::Vertex(v) => {
            ...
            for (k, vals) in v.properties {
                let py_vals = PyList::empty_bound(py);
                for val in vals {
                    py_vals.append(value_to_py(py, val)?)?;
                }
                props.set_item(k.to_string(), py_vals)?;
            }
...
impl PyTxSession {
    ...
    fn commit(mut slf: PyRefMut<'_, Self>) -> PyResult<()> {
        let session = slf.session.take().ok_or_else(|| {
            pyo3::exceptions::PyRuntimeError::new_err("Session already closed")
        })?;
        session.commit()
            .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("{:?}", e)))?;
