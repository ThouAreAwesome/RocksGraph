...
class TestVertexTraversals:
    ...
    def test_range(self, graph):
        ...
        items = rs.g().V().range(1, 4).values("n").to_list()
        assert len(items) == 3
    ...
    def test_fold_unfold_roundtrip(self, graph):
        ...
        folded = rs.g().V().values("x").fold().to_list()
        assert folded == [[1, 2]]

        # unfold flattens the list back
        unfolded = rs.g().V().values("x").fold().unfold().to_list()
    ...
    def test_coalesce_upsert(self, graph):
        ...
        found = (
            ...
            .has("email", "a@b.com")
            .fold()
            .coalesce(__.unfold(), __.addV("user").property("id", 999).property("email", "fallback"))
            .to_list()
        )
        assert len(found) == 1
...
class TestHashExistence:
    ...
    def test_has_key_filters(self, graph):
        ...
        total = rs.g().V().hasLabel("user").count().to_list()
        assert total == [2]
...
class TestInBothTraversals:
    def test_in_traversal(self, graph):
        ...
        neighbors = rs.g().V(v1["id"]).in_("knows").values("name").to_list()
...
class TestHasOnEdge:
    def test_has_key_on_edge(self, graph):
        ...
        assert all_edges == [2]
        with_weight = rs.g().V(v1["id"]).outE("knows").has("weight").count().to_list()
...
class TestUnion:
    def test_union(self, graph):
        ...
        rs = graph.read()
        result = rs.g().V(v1["id"]).union(rs.g().V(v1["id"]), rs.g().V(v2["id"])).values("name").to_list()
...
class TestRepeat:
    def test_repeat_out(self, graph):
        ...
        hop1 = rs.g().V(v1["id"]).repeat(__.out("link")).times(1).values("name").to_list()
        assert "B" in hop1

        # 2-hop from v1 should reach v3
        hop2 = rs.g().V(v1["id"]).repeat(__.out("link")).times(2).values("name").to_list()
        assert "C" in hop2
...
class TestV02SubTraversals:
    ...
    def test_simplePath(self, graph):
        ...
        result = rs.g().V(v1["id"]).out("link").simplePath().values("name").to_list()
        assert result == ["B"]
...
class TestChoose:
    def test_choose(self, graph):
        ...
        result = (
            ...
            .V()
            .hasLabel("person")
            .choose(__.has("age", P.gte(Int64(18))), __.constant("adult"), __.constant("minor"))
            .to_list()
        )
...
class TestGLVEnums:
    ...
    def test_T_label_same_as_string(self, graph):
        ...
        r2 = rs.g().V().hasLabel("item").order().by("label", "asc").values("age").to_list()
        assert r1 == r2
    ...
    def test_order_enum_asc(self, graph):
        ...
        result2 = rs.g().V().hasLabel("item").order().by("age", asc).values("age").to_list()
    ...
    def test_order_enum_desc(self, graph):
        ...
        result2 = rs.g().V().hasLabel("item").order().by("age", desc).values("age").to_list()
        assert result2 == [30, 20, 10]

    def test_order_by_value_standard_gremlin(self, graph):
        txn = graph.begin()
        for n in [30, 10, 20]:
            addv(txn, "item", age=Int64(n))
        txn.commit()
        rs = graph.read()
        # values("age").order().by(Order.Desc) or by(desc)
        result1 = rs.g().V().hasLabel("item").values("age").order().by(Order.Desc).to_list()
        assert result1 == [30, 20, 10]
        result2 = rs.g().V().hasLabel("item").values("age").order().by(desc).to_list()
        assert result2 == [30, 20, 10]
        result3 = rs.g().V().hasLabel("item").values("age").order().by(Order.Asc).to_list()
    ...
    def test_order_invalid_raises_error(self, graph):
        ...
        with pytest.raises(ValueError):
            rs.g().V().order().by("age", "invalid_order").to_list()
    ...
    def test_direction_out(self, graph):
        ...
        assert rs.g().V(v1["id"]).degree(Direction.In).to_list() == [0]
        assert rs.g().V(v1["id"]).degree(Direction.BOTH).to_list() == [1]
        assert rs.g().V(v1["id"]).degree(Direction.Both).to_list() == [1]
        assert rs.g().V(v1["id"]).degree().to_list() == [1]

    def test_direction_invalid_raises_error(self, graph):
        rs = graph.read()
        with pytest.raises(ValueError):
            rs.g().V(1).degree("invalid_dir").to_list()
...
class TestTxContextManager:
    ...
    def test_rollback_on_exception(self, graph):
        try:
            ...
            pass
        assert graph.read().g().V(9002).count().to_list() == [0]

    def test_exception_not_suppressed(self, graph):
        with pytest.raises(ValueError), graph.begin() as txn:
...
class TestExplain:
    def test_explain_read_traversal(self, graph):
        ...
        assert "PhysicalPlan" in plan_str
        assert "VStep" in plan_str
        assert "HasLabelStep" in plan_str

    def test_explain_tx_traversal(self, graph):
        with graph.begin() as txn:
            plan_str = txn.g().V().hasLabel("person").out("knows").explain()
            assert isinstance(plan_str, str)
            assert len(plan_str) > 0

    def test_explain_anonymous_traversal_raises(self):
        import pytest

        with pytest.raises(RuntimeError, match="Anonymous traversal cannot be explained"):
            __.V().hasLabel("person").explain()
