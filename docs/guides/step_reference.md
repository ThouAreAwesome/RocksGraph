...
- [Gremlin Traversal Language](#gremlin-traversal-language)
...
- [7. Branching & Control Flow](#7-branching--control-flow) — `repeat()`/`times()`/`until()`/`emit()`, `union()`, `coalesce()`
- [8. Plan Inspection: `explain()`](#8-plan-inspection-explain)
...
Gremlin is a **dataflow language for graphs**, not a declarative one like SQL. Instead of describing the shape of the result you want, you build a pipeline of composable steps and elements flow through it one at a time — the same mental model as Unix pipes (`cmd1 | cmd2 | cmd3`) or a functional `map`/`filter` chain. Reading left to right, `g.V([1]).out("knows").has("age", gt(25))` means: start at vertex 1, flow its outgoing `knows`-neighbors into the next step, then keep only the ones whose `age` exceeds 25. Nothing executes until a terminal step pulls results through the pipeline — everything before that just describes the dataflow's shape.

The object flowing through the pipeline at any given point is called a **traverser** — you'll see this term throughout the reference below. A traverser starts out as a vertex or edge, but steps like `.values()` or `.count()` replace it with a plain value (a string, a number) as it moves along. That's why each step's *Input → Output* type matters: what a later step receives depends entirely on what the step before it left the traverser holding.

Concretely, a traversal pipeline starts from an initial **Source Step** (`g.V()`, `g.E()`, `g.addV()`, `g.addE()`), passes through zero or more **Transform / Filter / Navigation Steps**, and is executed via a terminal step:
...
Gremlin itself comes from [Apache TinkerPop](https://tinkerpop.apache.org/), a graph computing framework used by several graph databases besides RocksGraph. This page documents the step vocabulary RocksGraph actually implements, not the full language — for the complete specification, concepts this reference doesn't cover, and other language variants (Groovy, Java, JavaScript), see the [official TinkerPop Reference Documentation](https://tinkerpop.apache.org/docs/current/reference/).
...
### Pattern: Fetch Only the Properties You Need (`withProperties()`)
By default, element traversals fetch **only `id` and `label`** — zero property I/O reads. Use `withProperties()` before the source step to opt in to property retrieval — see [Data Model, §6](data_model.md#6-property-retrieval--the-withproperties-hint) for the full explanation; the short version:
...
The hint isn't limited to the source step — it governs every element materialized downstream in the same traversal, including ones reached via navigation steps like `out()`:
- 🦀 Rust: `snap.g().withProperties([]).V([1]).out(["knows"]).to_list()?`
- 🐍 Python: `snap.g().withProperties().V(1).out("knows").to_list()`
