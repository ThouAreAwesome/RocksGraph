...
|------|-------------|-----------|
| Open with RocksDB tunables | `Graph::open_with_options(path, schema, storage_opts)` | User must import `RocksOptions`, learn CF names, understand memtable sizing |
...
## 2. Change 1: `RocksOptions` unified inside `GraphOptions`

Instead of creating multiple `open_with_*` methods (`open_with_options`, `open_with_storage_options`, `open_with_options`), all storage, runtime, and schema configurations are unified cleanly inside **`GraphOptions`**.
...
### `RocksOptions` definition

`RocksOptions` exposes only user-relevant hardware & storage tunables without leaky RocksDB jargon (no CF names, no BlockBasedTableFactory boilerplate), leaving room for separate `QueryOptions` in the future:
...
- **Pluggable backend abstraction layer.** RocksDB is the chosen, permanent storage engine. `RocksOptions` provides an ergonomic facade, avoiding leaky abstractions while retaining maximum engine performance.
- **Removing RocksDB power features.** Power users can use `RocksOptions.custom_rocks_modifier` to tweak low-level RocksDB parameters directly.
...
| Step | What | Effort |
|------|------|:---:|
| 1 | Add `RocksOptions` struct and embed inside `GraphOptions` | ~35 lines |
| 2 | Implement conversion from `RocksOptions` to RocksDB DB & CF options | ~40 lines |
| 3 | Deprecate `Graph::open_with_options` in favor of `Graph::open_with_options` | 1 attribute |
