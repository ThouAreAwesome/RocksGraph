...
[4]  g.read().g().nearest("embedding", q, k).to_list()
...
[3]  g.read().g().nearest("embedding", q, k).to_list()
...
[2]  for vid, text in all_docs():
     ...
     old = g.read().g().V([sample_id]).nearest("embedding",    q, 5).to_list()
     new = g.read().g().V([sample_id]).nearest("embedding_v2", q, 5).to_list()
...
[4]  g.read().g().nearest("embedding_v2", q, k).to_list()
