...
enum Merge {
    EndVertexIds(SmallVec<[VertexKey; SMALL_VECTOR_LENGTH]>),
    Rank(Rank),
}
...
fn intersect_ids(
    target: &mut Option<SmallVec<[VertexKey; SMALL_VECTOR_LENGTH]>>,
    incoming: SmallVec<[VertexKey; SMALL_VECTOR_LENGTH]>,
) {
    match target {
